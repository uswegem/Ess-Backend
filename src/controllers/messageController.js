...
class MessageController {
  ...
  static async resendMessage(req, res) {
    try {
      ...
      // Find the original message (scoped to tenant; platform-admin bypass falls through unscoped)
      let filter = { messageId };
      if (req.tenant?.tenantId) {
        filter = buildTenantQuery(req.tenant.tenantId, filter);
      }
      const originalMessage = await MessageLog.findOne(filter);
  ...
  static async getMessageById(req, res) {
    try {
      ...
      const message = await MessageLog.findOne(filter)
        .populate('sentBy', 'username fullName');
