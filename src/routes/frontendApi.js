...
router.get('/loan/status-statistics', async (req, res) => {
  try {
    ...
    const tenantMatch = req.tenant?.tenantId
    ...
    const rejectionStats = await LoanMapping.aggregate([
      { $match: { ...tenantMatch, status: 'REJECTED', rejectedBy: { $exists: true } } },
      {
        $group: {
          _id: '$rejectedBy',
          count: { $sum: 1 },
    ...
    const cancellationStats = await LoanMapping.aggregate([
      { $match: { ...tenantMatch, status: 'CANCELLED', cancelledBy: { $exists: true } } },
      {
        $group: {
          _id: '$cancelledBy',
          count: { $sum: 1 },
