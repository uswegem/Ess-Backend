 ...
 *   get:
 *     summary: Get suggested manually-triggerable message types for a loan
 *     description: Returns additive suggestions based on loan status and message history; does not restrict what can be sent.
 *     tags: [Loan Actions]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         required: true
 *         schema: { type: string }
 *         description: ESS application number, ESS loan number alias, or Mifos loan ID
 *     responses:
 *       200:
 *         description: Suggested and full message-type list for this loan
 *       404:
 *         description: Loan mapping not found
 */
router.get('/:loanId/suggested-messages', authMiddleware, permissionMiddleware('messages:trigger'), async (req, res) => {
    try {
        const { loanId } = req.params;
        const tenantId = req.tenant?.tenantId || null;

        const result = await LoanMappingService.getSuggestedMessages(loanId, tenantId);
        ...
        res.status(500).json({ success: false, message: error.message || 'Failed to get suggested messages' });
    }
});
